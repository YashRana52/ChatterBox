...
function Connection() {
  ...
  const activeTabData = tabs.find((item) => item.label === currentTab);
  const usersToShow = activeTabData?.value || [];
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="mb-8 flex flex-wrap gap-6">
          {tabs.map((item, index) => (
            <div
              key={index}
              className="flex flex-col items-center justify-center gap-1 h-20 w-40 border border-gray-200 bg-white shadow rounded-md"
            >
        ...
        <div className="inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm">
          {tabs.map((tab) => (
            <button
              ...
              key={tab.label}
              aria-pressed={currentTab === tab.label}
              className={`cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
        ...
        <div className="flex flex-wrap gap-6 mt-6">
          {usersToShow.length === 0 ? (
            <p className="text-slate-600">No {currentTab.toLowerCase()} yet.</p>
          ) : (
